//! Example: path analysis on a small in-memory network.
//!
//! Same diamond network as `simple_network` / `multiclass_network`,
//! but with `store_paths = true` to extract per-OD shortest paths.
//!
//! Demonstrates:
//! 1. Extracting the shortest route between two zones (OD pair query)
//! 2. Select link analysis (which OD pairs use a given link)
//!
//! ```text
//!        Zone 1 (residential)
//!          |
...
fn main() {
    ...
    // Config: Frank-Wolfe with store_paths, 3 feedback iterations
    ...
    // 1) OD pair query: shortest route from Zone 1 to Zone 4
    let origin = 1;
    let dest = 4;

    let mut od_paths: Vec<_> = paths
        .iter()
        .filter(|p| p.origin_zone == origin && p.dest_zone == dest)
        .collect();
    od_paths.sort_by(|a, b| b.flow.partial_cmp(&a.flow).unwrap());

    let total_flow: f64 = od_paths.iter().map(|p| p.flow).sum();
